...
import { createBuilding, listBuildings, listBuildingsForOrganization, DbDisabledError } from '@/lib/school-foundation/repos';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rows = organizationId
      ? await listBuildingsForOrganization(Number(organizationId))
      : await listBuildings(siteId ? Number(siteId) : undefined);
    return json(rows);
  } catch (e) {
