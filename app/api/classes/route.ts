...
import { createClass, listClasses, DbDisabledError } from '@/lib/school-foundation/repos';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rows = await listClasses(organizationId ?? undefined);
    return json(rows);
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const row = await createClass(body);
    return json(row, 201);
  } catch (e) {
    if (e instanceof DbDisabledError) return err(e.message, 503);
