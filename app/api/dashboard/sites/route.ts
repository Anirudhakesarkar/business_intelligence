...
export async function POST(request: NextRequest) {
  try {
    ...
    const { serverUrl, siteId, apiKey } = body as { serverUrl?: string; siteId?: string; apiKey?: string };

    if (!serverUrl || !siteId || !apiKey) {
      return NextResponse.json({ error: 'serverUrl, siteId, and apiKey are required' }, { status: 400 });
    }
    ...
    const url = `${base}/v1/dashboard/sites`;
